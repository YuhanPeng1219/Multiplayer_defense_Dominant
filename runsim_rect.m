...
%% Target
target = shapeRectangleByCenter( g, [-1.5;-.7], [.3;0.4] );
[t_interp, g_new] = gridInterpolation(g, target, 400);

%% Players
xd = [-0.9,-0.68]; xa = [1.5,-0.1];
...
[t_bound,~] = contour(g_new.xs{1},g_new.xs{2},t_interp,[0 0],'r','linewidth',2);
t_bound(:,1) = [];
...
% Reach or not
value = eval_u(g_new, dmat_interp, t_bound);
if all(value<0)
    disp('Attacker cannot reach')
    % interpolate
    [~,matG,~,~, ~] = compute_isodata(target,params);
    matG(obstacles<0) = nan;
    
    [matG_interp, ~] = gridInterpolation(g, matG, interp_N);
    
    t_v = eval_u(g_new, matG_interp, bound);
    
    [~, ind] = min(t_v);
    opt = bound(:,ind);
    hs = scatter(opt(1),opt(2));
else
...
while iter <= tauLength
    ...
    for i = 1:2
        ...
        set(hplayer{i}, 'XData', xs(i,1), 'YData', xs(i,2));
    end
    ...
    opt = optCal(dmat_interp, t_bound, matG_interp, g_new, bound);
    set(hs, 'XData', opt(1), 'Ydata', opt(2));
    opt_list = [opt_list,opt];
    
    % if opt change, recompute reachable set
    d = norm(opt - opt_list(:,end-1));
    if d>small
        D = vecnorm(opt-opt_list(:,1:end-1));
        index = find(D<small);
        
        if isempty(index)
            [data,mat,vx,vy,~] = compute_pointdata(opt,params);
            data_f = flip(data,3);
            data_ls{iter+1}=data_f;
        else
            for o=1:length(index)
                if ~isempty(data_ls{index(o)})
                    inde = index(o);
                    break;
                end
            end
...
%% save data for video
save('data/video_1v1_rect_stable', 'bound', 'dmat_list', 'g', 'g_new','params',...
